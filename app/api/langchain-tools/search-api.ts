...
export class SearchApi extends Tool {
  ...
  constructor(
    ...
    if (!baseUrl) {
      throw new Error(
        "SearchApi requires an API key. Please set it as SEARCHAPI_BASE_URL in your .env file, or pass it as a parameter to the SearchApi constructor.",
      );
    }
  ...
  protected buildUrl(searchQuery: string): string {
    ...
    return `${this.baseUrl}/searchapi/search?${searchParams}`;
  }
